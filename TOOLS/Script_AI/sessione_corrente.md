# Sessione corrente — 2026-05-31

## PROGETTO ATTIVO
Reel Ferrari pubblicato su Facebook Codice Reale.

## PROSSIMO PASSO IMMEDIATO
Prossimo Reel: aggiungere la musica **"Nuvolari" di Lucio Dalla** dalla libreria Facebook.
Sequenza corretta: carica video → cerca "Nuvolari" nella libreria musicale → poi vai avanti con didascalia.

## DA FINIRE — sessione prossima
- Pagina Facebook Quantum Srl (da creare)
- Pubblicare Reel anche su Instagram @codicereale
- Brand SOSTANZA — dominio da verificare su register.it + aprire Payhip
- Sito Oscar — ottimizzare altezza card sezione "Chi sono"
- Video Lucio (scherzo HeyGen) — generare dopo prova audio

## SCADENZE IMMINENTI
- **02/06/2026** — Chiamare Di Salvatore (istanza GE v5 Mobilcom)
- **02/06/2026** — Parlare con Pichetti su EVOLUTIAA
- **03/06/2026** — Arte Sacra: risposta a Severino (fideiussione)
- **03/06/2026** — Zero to AI call ore 16:00 (Google Meet)
- **30/06/2026 ore 12:00** — Termine offerte asta Mobilcom
- **01/07/2026** — Asta Mobilcom

## APPESO IN ARIA
- EVOLUTIAA — decisione entro 3 giugno, prima parlare con Pichetti
- Arte Sacra fideiussione — tempo fino al 3 giugno
- SAUDA (Mobilcom) — parlarne prima con Di Salvatore
- Sito Quantum — attesa credenziali hosting da Pichetti
- Brand SOSTANZA — dominio da verificare su register.it + aprire Payhip
- Sito Oscar — ottimizzare altezza card sezione "Chi sono"
- Minifrigo bar ufficio — scelta finale: OK OFK 061 E W 89,99€ MediaWorld Corridonia o PremierTech PT-F47 118,90€ Amazon

## FATTO IN QUESTA SESSIONE (31/05)
- Reel Ferrari pubblicato su Facebook Codice Reale (senza musica — da aggiungere al prossimo)
- Google Search Console verificata con Tag HTML
- Sitemap.xml con 4 pagine inviata a Google
- Guida Facebook 2026 salvata in REPUTAZIONE_SOCIAL
- Video Ferrari MP4 generato con Playwright (converti_video_pw.js) — funziona correttamente
- Ricerca minifrigo bar completa: prezzi verificati su Amazon, MediaWorld, Euronics, OBI, Unieuro, eBay
- Copertine social Quantum create (LinkedIn 1128x191, Facebook 1920x711, YouTube 2560x1440)
- Descrizioni aziendali e post di lancio Pichetti scritti
- Istruzioni apertura pagine Facebook/LinkedIn/YouTube salvate
- Tutto in: QUANTUM\Marketing\Materiale_Grafico\Copertine_Social\

## NOTA MUSICA PROSSIMO REEL
Facebook permette di aggiungere musica solo nella prima schermata di creazione, prima di "Modifica reel".
Canzone scelta: **"Nuvolari" di Lucio Dalla** — cercarla nella libreria musicale Facebook al momento del caricamento.
